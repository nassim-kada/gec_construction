...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* SUIVI */}
      <section id="suivi" className="py-24 relative overflow-hidden" ref={suiviRef as any}>
        <motion.div 
          className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-16 bg-primary"
          initial={{ scaleY: 0 }}
          animate={suiviVisible ? { scaleY: 1 } : {}}
          transition={{ duration: 1 }}
          style={{ transformOrigin: "top" }}
        />
        <div className="container mx-auto px-4 pt-8">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={suiviVisible ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.6 }}
            className="text-center mb-16"
          >
            <h2 className="font-serif text-3xl md:text-5xl text-foreground mb-4">Suivi à Distance</h2>
            <div className="w-24 h-1 bg-primary mx-auto"></div>
          </motion.div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16 max-w-4xl mx-auto">
            {[
              { icon: "📸", t: "Photos hebdomadaires" },
              { icon: "📹", t: "Vidéos du chantier" },
              { icon: "📊", t: "Rapports détaillés" },
              { icon: "💬", t: "Communication directe" }
            ].map((item, i) => (
              <motion.div 
                key={i}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="flex flex-col items-center text-center p-6 bg-card border border-border"
              >
                <div className="text-4xl mb-4">{item.icon}</div>
                <div className="text-sm font-medium tracking-wide">{item.t}</div>
              </motion.div>
            ))}
          </div>
          ...
          <motion.div 
            ...
            className="max-w-4xl mx-auto bg-[#0A0A0A] border border-primary/20 rounded-lg overflow-hidden shadow-[0_0_50px_rgba(201,168,76,0.05)]"
          >
            <div className="bg-[#111] border-b border-border p-4 flex items-center justify-between">
              <div className="flex gap-2">
                <div className="w-3 h-3 rounded-full bg-border"></div>
                <div className="w-3 h-3 rounded-full bg-border"></div>
                <div className="w-3 h-3 rounded-full bg-border"></div>
              </div>
              <div className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Portail Client</div>
              <div className="w-12"></div> {/* spacer */}
            </div>
            
            <div className="p-6 md:p-8 flex flex-col gap-6">
              {[
                { name: "Villa Bab Ezzouar", phase: "Gros Œuvre", progress: 78, status: "En cours", sColor: "text-primary border-primary" },
                { name: "Appartement Hydra", phase: "Second Œuvre", progress: 45, status: "En pause", sColor: "text-destructive border-destructive" },
                { name: "Rénovation Chéraga", phase: "Livraison", progress: 92, status: "Bientôt terminé", sColor: "text-green-500 border-green-500" }
              ].map((proj, i) => (
                <div key={i} className="bg-card p-4 md:p-6 rounded border border-border flex flex-col gap-4">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <h4 className="font-serif text-lg text-foreground">{proj.name}</h4>
                      <span className="text-sm text-muted-foreground">{proj.phase}</span>
                    </div>
                    <div className={`px-3 py-1 text-xs border rounded-full uppercase tracking-wider ${proj.sColor} inline-block w-max`}>
                      {proj.status}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    <div className="flex-1 h-2 bg-background rounded-full overflow-hidden">
                      <motion.div 
                        initial={{ width: 0 }}
                        animate={suiviVisible ? { width: `${proj.progress}%` } : {}}
                        transition={{ duration: 1.5, delay: 0.8 + i * 0.2, ease: "easeOut" }}
                        className="h-full bg-gradient-to-r from-primary to-primary/80"
                      />
                    </div>
                    <span className="font-mono text-sm text-primary w-10 text-right">
                      <AnimatedCounter end={proj.progress} duration={1.5} suffix="%" />
                    </span>
                  </div>
                </div>
              ))}
      ...
      <footer className="bg-background pt-16 pb-8 border-t border-border">
        <div className="container mx-auto px-4 text-center">
          ...
          <nav className="flex flex-wrap justify-center gap-6 md:gap-12 mb-12">
            {["Engagements", "Services", "Partenaires", "Suivi", "Contact"].map((link) => (
